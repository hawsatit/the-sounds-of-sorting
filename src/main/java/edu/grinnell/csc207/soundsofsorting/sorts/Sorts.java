...
public class Sorts {
    ...
    /**
     * Compares the root value to the branches and swaps it if the branches 
     * are larger than the root and keeps recursion down until no swap is made
     * @param <T> the carrier type of the array
     * @param arr the array to sort
     * @param n number of elements being compared
     * @param root the root value being compared
     */
    public static <T extends Comparable<? super T>> void makeHeap(T[] arr, int n, int root) {
      ...
      if (left < n && arr[left].compareTo(arr[root]) > 0){
          swap(arr, root, left);
          makeHeap(arr, n, left);
      }
      
      if (right < n && arr[right].compareTo(arr[root]) > 0){
          swap(arr, root, right);
          makeHeap(arr, n, right);
      } 
